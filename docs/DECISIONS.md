...
| Back Office | État réel d'un fournisseur (santé toujours `operational` en SANDBOX, faute d'Availability Engine, Prompt 47) ; state machine KYC réelle (`adminUpdateKycStatus` n'exige qu'un changement, aucune transition validée — ADR-050, probablement lié à un futur fournisseur KYC externe) ; contenu Countries/FAQ/Legal jamais consommé ailleurs dans le produit (aucun sélecteur pays, aucune page CGU/confidentialité publique — ADR-050) ; Reconciliation et Incidents restent `ComingSoonPage`, aucun moteur sous-jacent (Prompt 24 et Prompt 47 respectivement) |
| RBAC | Correspondance rôle → permissions non documentée dans les sources, choix assumé (ADR-051) ; `transaction.review`/`transaction.cancel` déclarées mais jamais câblées à une action réelle (aucun workflow de révision/annulation manuelle, probablement Prompt 24) ; aucune notification à un utilisateur retiré de tous ses rôles |
