...
| Assist | Véritable compréhension du langage naturel / intégration LLM (le pattern-matching de `intent-detector.ts` est un point de départ raisonnable, pas une NLU — ADR-049) ; contenu FAQ plus riche que les six thèmes de guidage actuels ; module Back Office Support pour traiter les `support_tickets` créés (Prompt 22, aucun agent humain ne peut encore les prendre en charge côté produit) ; réponse asynchrone d'un agent humain visible côté utilisateur (statut du ticket jamais notifié — dépendrait aussi du Notification Engine, Prompt 20, si câblé plus tard) |
